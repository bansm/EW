...
using EW.Core;
using EW.Utility.Exceptions;
using EW.Utility.Extensions;
using EW.Utility.Logging;
using EW.Core.Data;
using EW.Core.Logging;
using EW.Data.Extensions;
...
namespace EW.Data
{
    ...
    /// </summary>
    public class EWDbContext : DbContext, IUnitOfWork, IDependency
    {
        private static readonly ILogger Logger = LogManager.GetLogger(typeof(EWDbContext));
        ...
        /// </summary>
        public EWDbContext(string nameOrConnectionString)
        ...
        {
            string name = ConfigurationManager.AppSettings.Get("Conn") ?? "default";
            return name;
        }
        private static string GetConnectionStringName(string Conn)
        {
            string name = ConfigurationManager.AppSettings.Get(Conn) ?? "default";
            return name;
        }
        ...
        private static bool DataLoggingEnabled
        {
            get
            {
                return ConfigurationManager.AppSettings.Get("DataLoggingEnabled").CastTo(false);
            }
        }
        ...
        public int ExecuteSqlCommand(EW.Core.Data.TransactionalBehavior transactionalBehavior, string sql, params object[] parameters)
        {
            System.Data.Entity.TransactionalBehavior behavior = transactionalBehavior == EW.Core.Data.TransactionalBehavior.DoNotEnsureTransaction
        ...
        internal int SaveChanges(bool validateOnSaveEnabled)
        {
            ...
            catch (DbUpdateException e)
            {
                if (e.InnerException != null && e.InnerException.InnerException is SqlException)
                {
                    ...
                    throw new EWException("提交数据更新时发生异常：" + msg, sqlEx);
                }
                throw;
            }
            finally
        ...
        /// <param name="parameters">要应用于命令字符串的参数。</param>
        /// <returns>执行命令后由数据库返回的结果。</returns>
        public async Task<int> ExecuteSqlCommandAsync(EW.Core.Data.TransactionalBehavior transactionalBehavior, string sql, params object[] parameters)
        {
            System.Data.Entity.TransactionalBehavior behavior = transactionalBehavior == EW.Core.Data.TransactionalBehavior.DoNotEnsureTransaction
                ? System.Data.Entity.TransactionalBehavior.DoNotEnsureTransaction
                : System.Data.Entity.TransactionalBehavior.EnsureTransaction;
